...
public class UserDaoImplTest
{
    ...
    {
        User user = getUser();
        Integer id = userDao.saveUserEntity(user);
    ...
    {
        User user = userDao.getUserByName("杜玉萍");
    ...
    {
        User user = userDao.getUserByNo("114");
    ...
    private User getUser() throws ParseException
    {
        ...
        user.setUser_no("114");
        user.setUser_pwd("efafa13");
        user.setUser_sex("男");
        Date bir = formatter.parse("1990-06-03 00:00:00");
        user.setUser_birth(bir);
        user.setUser_class("95035");
        Date cur = new Date();
        user.setDate_create(cur);
        cur.setDate(-1);
        user.setDate_update(cur);
        return user;
    }
}